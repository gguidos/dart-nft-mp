...
export async function deleteKey(cacheKey) {
  let cache = new CacheManager(cacheKey);
  await cache.delKey()
...
export async function setCache(cacheKey, data, ttl?) {
...
export async function getKeys(cacheKey) {
  let cache = new CacheManager(cacheKey);
  let keys = await cache.getKeys();
...
export async function addArrayToCache(cacheKey, data) {
  let cache = new CacheManager(cacheKey);
  let keys = await cache.getKeys();
  if (keys && keys.length > 0) {
...
export async function addElementToDictionary(cacheKey, data) {
  let cache = new CacheManager(cacheKey);
  let keys = await cache.getKeys();
  if (keys.length > 0) {
    let cacheRes = await cache.getCache();
...
export async function addObjectToCacheById(cacheKey, object) {
 let cache = new CacheManager(cacheKey);
 let keys = await cache.getKeys();
...
export async function cleanIdFromCache(cacheKey, id, ttl?) {
  let cache = new CacheManager(cacheKey, ttl);
  let keys = await cache.getKeys();
  if (keys.length > 0) {
    let cacheRes = await cache.getCache();
...
export async function cleanObjectFromCache(cacheKey, id, ttl?) {
  let cache = new CacheManager(cacheKey, ttl);
  let keys = await cache.getKeys();
  if (keys.length > 0) {
    let cacheRes = await cache.getCache();
...
export async function addElementToList(cacheKey, element, ttl?) {
  let cacheData = [];
  let cache = new CacheManager(cacheKey, ttl);
  let keys = await cache.getKeys();
  if (keys && keys.length > 0) {
